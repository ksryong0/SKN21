-- 한줄 주석 (--공백 으로 시작)
/* block 주석 */

/*
Mysql Workbench
  - font 바꾸기 - edit/preferences -> font and color
  
  - SQL문 실행 - control + enter (cmd + enter)
*/


/**********************************************************************************************************
사용자 계정 생성
- create user 'username'@'host' identified by 'password'
  - usernamer과 host를 따로 작은 따옴표로 묶어준다.
  - host
    - localhost :로컬접속 계정
    - % : 원격 접속 계정

**********************************************************************************************************/
drop user 'playdata'@'%';

-- local 접속 계정
create user 'playdata'@'localhost' identified by '1111';
-- 원격 접속 계정
create user 'playdata'@'%' identified by '1111';
-- 등록된 사용자계정 조회


/********************************************************
 계정에 권한 부여
- GRANT 부여할 권한 ON 데이터베이스.테이블 TO 계정@host
- 데이터베이스와 테이블을 `*` 로 지정하면 모든 DB와 테이블에 적용된다.
- 주요 권한 목록
  - all privileges: 모든권한
  - 테이블의 데이터 관리: select, insert, update, delete
  - DB 객체 관리: create, drop, alter
  - 사용자관리: create user, drop user, grant option
********************************************************/
FLUSH PRIVILEGES;
GRANT ALL PRIVILEGES ON *.* TO 'playdata'@'localhost';


use testdb;


/***********************************************************************************************************
테이블 생성
create table 테이블 이름 (컬럼명  데이터타입  [제약조건])

테이블 삭제
drop table [if exists] 삭제할테이블이름

------------------------------------------------------------------------------------------------------------
테이블: 회원 (member)
속성
id:        varchar(10)    primary key
password:  varchar(10)    not null (필수)
name:      varchar(30)    not null
point:     int            nullable
email:     varchar(100)   unique key
gender:    char(1)        not null, check key - 'm', 'f' 만 값으로 가진다.
age:       int            check key - 양수만 값으로 가진다.
join_date: timestamp      not null, 기본값-값 저장시 일시
***********************************************************************************************************/
-- 테이블 생성
create table member(
	id varchar(10) PRIMARY KEY,
    password varchar(10) NOT NULL,
    name varchar(30) NOT NULL,
    point int,
    email varchar(100) unique key,
    gender char(1) NOT NULL check(gender in ('m', 'f')),
    age int check(age > 0),
    join_date timestamp NOT NULL default current_timestamp
);
-- 테이블 조회
show tables;

-- 테이블 상세정보 조회(컬럼)
describe member;
desc member; -- 위와 같은 코드

-- 테이블 삭제
drop table if exists member; -- 테이블이 있으면 삭제. 없으면 경고

-- DML : INSERT, UPDATE, DELETE
-- DQL : SELECT


/* *********************************************************************
INSERT 문 - 행 추가
구문
 - 한행추가 :
   - INSERT INTO 테이블명 (컬럼 [, 컬럼]) VALUES (값 [, 값[])
     - 모든 컬럼에 값을 넣을 경우 컬럼 지정구문은 생략 할 수 있다.
     - SQL은 기본적으로 한 행(한개의 데이터)씩 테이블에 추가한다.
     - 특정 컬럼에만 값을 넣을 수 있다.

************************************************************************ */
INSERT INTO member (id, password, name, point, 
	email, gender, age, join_date) 
	VALUES ('ryongryong', '1234', 'ryong', 40, 
    'ksryong@gmail.com', 'm', 39, '2025-10-14 17:00:00');
    
INSERT INTO member
	VALUES ('ryoryong2', '12354', 'ryon2g', 402, 'ksryo2ng@gmail.com', 'f', 390, '2025-10-14 17:24:00');

-- 특정 컬럼에만 값을 넣을 수 있다.    
INSERT INTO member (id, password, name, gender)
	VALUES ('ryoryong3', '12354', 'ryon2g', 'f');

INSERT INTO member (id, password, name, point, 
	email, gender, age, join_date) 
	VALUES ('ryongrng4', '1234', 'ryong', 40, 
    'ksryg3@gmail.com', 'm', NULL, '2025-10-14 17:00:00');
    
-- 타임스탬프에 날짜만 insert하고 시, 분, 초를 안넣으면 00:00:00 로 들어감
INSERT INTO member (id, password, name, gender, age, join_date)
	VALUES ('ryoryong5', '12354', 'ryon2g', 'f', 20, '2000-10-10');

-- password를 not null로 설정해서 null을 넣을 수 없음.
INSERT INTO member (id, password, name, gender)
	VALUES ('ryoryong3', null, 'ryon2g', 'f');