...
SELECT DISTINCT job, emp_name
	FROM emp;

-- EMP 테이블에서 emp_id는 직원ID, emp_name은 직원이름, hire_date는 입사일, salary는 급여, dept_name은 소속부서 별칭으로 조회결과를 출력 한다.
-- UPPER(컬럼명) 을 하면 결과값을 대문자로 바꿔줌
SELECT emp_id AS '직원ID', emp_name AS '직원이름', hire_date AS '입사일', 
	salary AS '급여', UPPER(dept_name) AS '소속부서'
    FROM emp;
    
-- AS는 생략 가능!!!
SELECT emp_id '직원ID', emp_name '직원이름', hire_date '입사일', 
	salary '급여', UPPER(dept_name) '소속부서'
    FROM emp;
...
- 여러개 값을 합쳐 문자열로 반환
	- concat(값, 값, ...)
- 피연산자가 null인 경우 결과는 null. ex)1+2+3+NULL 하면 결과는 NULL 이다.
- 연산은 그 컬럼의 모든 값들에 일률적으로 적용된다.
- 같은 컬럼을 여러번 조회할 수 있다. SELECT salary, salary, salary FROM emp; 요런식으로 가능.
************************************** */

-- 컬럼별로 다른 처리
SELECT emp_name as '직원이름', salary as '월급', dept_name as '부서',
	CASE
		WHEN dept_name = 'Shipping' THEN salary * 5
        WHEN dept_name = 'Sales' THEN salary * 10
	END AS 성과
	FROM emp
    WHERE dept_name IN ('Shipping', 'Sales');

-- EMP 테이블에서 직원의 이름(emp_name), 급여(salary), 급여(salary)을 연봉으로 조회. (곱하기 12)
SELECT emp_name as '직원이름', salary as '월급', salary*12 as '연봉'
	FROM emp;

-- EMP 테이블의 직원의 이름(emp_name), 급여(salary)를 조회하는데 급여 앞에 '$'를 붙여서 조회한다.
SELECT emp_name, concat('$',salary,'원') as 'salary'
	FROM emp;

-- EMP 테이블에서 직원의 ID(emp_id), 이름(emp_name), 급여(salary), 커미션_PCT(comm_pct), 급여에 커미션_PCT를 곱한 값을 조회.
SELECT emp_id as 'ID', emp_name as '이름', salary as '급여', comm_pct as '커미션_PCT', salary*comm_pct as '급여 x 커미션' 
	FROM emp;
...
SELECT emp_name 직원이름, dept_name 부서명
	FROM emp
    WHERE emp_id = 110;
 
-- EMP 테이블에서 'Sales' 부서에 속하지 않은 직원들의 ID(emp_id), 이름(emp_name),  부서명(dept_name)을 조회.
SELECT emp_id AS "ID", emp_name AS "이름", dept_name AS "부서명"
	FROM emp
	WHERE dept_name NOT IN("Sales");
...
SELECT emp_name
	FROM emp
    WHERE emp_name LIKE '__e%'; -- __e만 하면 되는게 아니고 뒤에 %를 붙여줘야됨.
...
SELECT emp_id, emp_name
	FROM emp
    WHERE emp_name LIKE '%&%%' escape '&';	-- escape '특수문자'. 특수문자뒤에 %나 _는 조회할 값임을 표시.(like의 패턴문자가 아니라)

-- EMP 테이블에서 부서명(dept_name)이 null인 직원의 ID(emp_id), 이름(emp_name), 부서명(dept_name)을 조회.
SELECT emp_id, emp_name, dept_name
	FROM emp
    WHERE dept_name IS NULL; -- WHERE 절에서 NULL인 값을 조회하고 싶을 때는 = NULL이 아니고 IS NULL로 조회해야함
...
SELECT *
	FROM (SELECT * FROM emp WHERE job LIKE '%MAN%') as a
    WHERE dept_name = 'Shipping' AND year(hire_date) >= 2005;
...
-- AND랑 OR가 같이 있으면 연산자 우선순위가 AND가 우선임. 무조건 AND 먼저함.
SELECT emp_id, emp_name, job, dept_name
	FROM emp
    WHERE job LIKE '%MAN%' 
...
-- 이렇게 하면 job like문이랑 dept_name을 AND를 먼저하고 OR문을 처리함
SELECT emp_id, emp_name, job, dept_name
	FROM emp
    WHERE job LIKE '%MAN%' 
    AND dept_name = 'Marketing'
...
Date 오름차순 : 과거 -> 미래
null은 오름차순일 때 가장 먼저 나온다.(문자든 숫자든 상관없이)
...
-- 컬럼의 이름 대신에 컬럼의 순번으로 ORDER BY 가능
SELECT emp_id, emp_name, salary
	FROM emp
    WHERE salary >= 5000
    ORDER BY 3 DESC;