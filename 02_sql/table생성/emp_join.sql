...
CREATE TABLE emp(
    ...
    -- CONSTRAINT 포린키 이름 FOREIGN KEY(포린키로 설정할 자식 테이블의 컬럼) REFERENCES 참조할 부모 테이블이름(참조할 부모 테이블의 컬럼명)
    -- ON DELETE SET NULL : 부모 컬럼에서 삭제하면 자식 컬럼의 데이터는 NULL로 바꿔주겠다.
    -- ON DELETE CASCADE : 부모 컬럼에서 삭제하면 걔를 참조하는 자식 데이터도 삭제하겠다.
    -- **자식 테이블에 INSERT INTO로 값을 추가할 때, 참조하는 부모테이블의 데이터로 존재하는 값만 추가할 수 있음. 부모테이블에 없으면 에러 남!
