...
SELECT char_length(trim('     aaaa  ')) "공백 제거 후, 문자 길이 구하기";

-- EMP 테이블에서 직원의 이름(emp_name)을 모두 대문자, 소문자, 이름 글자수를 조회
SELECT UPPER(emp_name) "대문자", LOWER(emp_name) "소문자", char_length(emp_name) "이름 글자수"
	FROM emp;

-- 직원 이름(emp_name) 의 자릿수를 15자리로 맞추고 15자가 안되는 이름의 경우  공백을 앞에 붙여 조회. 
SELECT lpad(emp_name, 15, ' '), rpad(emp_name, 5, ' ')
	FROM emp;
    
--  EMP 테이블에서 이름(emp_name)이 10글자 이상인 직원들의 이름(emp_name)과 이름의 글자수 조회
SELECT emp_name, char_length(emp_name) AS "이름 글자수"
	FROM emp
...
************************************************************************** */
select round(12345.12345, 2); #자리 2 이하에서 반올림. 실수부
...
SELECT emp_id "직원ID", emp_name "이름", 
	salary "급여", ceil(salary*1.15) "SAL_RAISE", floor(salary*1.15) "SAL_DOWN"
	FROM emp;

-- 위의 SQL문에서 인상 급여(sal_raise)와 급여(salary) 간의 차액을 추가로 조회 
-- (직원ID(emp_id), 이름(emp_name), 15% 인상급여, 인상된 급여와 기존 급여(salary)와 차액)
SELECT emp_id "직원ID", emp_name "이름", round(salary*1.15) "인상된급여", salary "기존급여",
	round(salary*1.15)-salary "인상된 급여와 기존 급여(salary)와 차액"
	FROM emp;
...
SELECT emp_id "직원_ID", emp_name "이름", comm_pct "커미션비율", round(comm_pct*1.08, 2) "comm_raise",
	truncate(comm_pct*1.08, 2) "내림"
	FROM emp
...
- 날짜관련 함수
 ...
 hour(시간), minute(시간), second(시간), microsecond(시간): 시간 또는 일시의 시, 분, 초, 마이크로초를 반환한다.
...
-- 시간 타입에서 시 분 초 조회
SELECT curtime();
SELECT hour(now()) "시", minute(now()) "분", second(now()) "초";
--
...
SELECT adddate(now(), interval 10 week); -- 3주후

SELECT avg(salary) 
	FROM emp;

SELECT DATEDIFF(curdate(), '2024-10-10');
SELECT TIMEDIFF(curtime(), '10:10:10');
SELECT timestampdiff(month, '2024-10-10', now()); -- 앞 날짜부터 뒤 날짜까지 몇개월이 지났나? month로 물어봤으니까
SELECT timestampdiff(hour, '2024-10-10', now());-- 앞 날짜부터 뒤 날짜까지 몇시간이 지났나?
-- EMP 테이블에서 부서이름(dept_name)이 'IT'인 직원들의 '입사일(hire_date)로 부터 10일전', 입사일, '입사일로 부터 10일 후' 의 날짜를 조회. 
SELECT subdate(hire_date, interval 10 day) "입사 10일전 날짜",
	hire_date,
    adddate(hire_date, interval 10 day) "입사 10일후 날짜"
    FROM emp
    where dept_name = "IT";
...
SELECT date_format(now(), "%Y년 %m월 %d일 %H시 %i분 %s초") "현재 날짜 및 시간";
...
SELECT emp_name, timestampdiff(month, hire_date, curdate()) "근무개월수"
	FROM emp
    ORDER BY 2 DESC; -- 만들 컬럼의 별명으로 order by 는 안됨. 컬럼 번호로 order by 해야함.
...
SELECT emp_id, emp_name, job, ifnull(dept_name, '배치전')
	FROM emp;
...
SELECT emp_id, emp_name, job, 
	CASE job
		WHEN job = 'AD_PRES' THEN '대표'
        WHEN job = 'FI_ACCOUNT' THEN '회계'
        WHEN job = 'PU_CLERK' THEN '구매'
        ELSE job -- 출력값에 column명을 작성하면 원래 값을 출력한다.
    END AS "한글 직업"
	FROM emp
...
SELECT dept_name, salary, 
		CASE
			WHEN dept_name = 'IT' THEN salary * 0.1
