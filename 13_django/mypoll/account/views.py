...
from django.contrib.auth.forms import (
    ...
    PasswordChangeForm  # 패스워드 변경 화면의 폼
)
...
def create(request):
    ...
    if request.method == "GET":
        ...
        if form.is_valid(): #요청 파라미터에 문제가 없는 경우
            ...
            user = form.save() # 반환: save() 처리한 Model객체를 반환.
...
def detail(request):
    try:
        ...
        return render(request, "error.html", {"error_message":"회원정보 조회도중 문제가 발생."})
    ...
    if request.method == "GET":
        ...
        ## 일치 - 로그인 처리(session에 로그인 사용자 정보-UserModel-을 저장)
            ...
            return render(
                ...
                "account/login.html",
                {"form":AuthenticationForm(), 
                 "error_msg":"username, password를 다시 확인 하세요."}
            )
...
# 처리 - GET:패스워드 변경 폼 페이지 이동(account/password_change.tml)
