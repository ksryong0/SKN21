...
## 그 질문과 보기를 조회해서 화면에 출력 - 설문 입력 폼

### 기존 request(요청) parameter: http://ip:port/path1/path2/path3?name=전달값1&name=전달값2
...
def vote_form(request, question_id):
    ...
    # view함수의 두번쨰 파라미터부터는 path parameter를 받을 변수들.
...
# 요청 파라미터: GET - request.GET -> dictionary {"요청파라미터이름":"요청파라미터값"}
