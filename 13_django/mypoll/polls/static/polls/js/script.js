function addChoice() 
{
    // "input[name$='choice_text']" : input태그 조회. name속성의 값이 choice_text로 끝나는 것
    // "input[name^='choice_text']" : input태그 조회. name속성의 값이 choice_text로 시작하는 것
    ...
    // formset으로 생성한 TOTAL_FORMS의 값을 1 증가
...
function delChoice()
{
    ...
    if (layer.children.length > 2)
    {
        ...
        // formset으로 생성한 TOTAL_FORMS의 값을 1 감소
        const totalForms= document.getElementById("id_form-TOTAL_FORMS")
        totalForms.value = parseInt(totalForms.value) - 1
    }
    else
