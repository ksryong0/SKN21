...
class Question(models.Model):
    ...
    #             Field 객체를 할당. Field객체 - column 설정(type, null 허용여부)
    question_text = models.CharField(max_length=200) # CharField() -> 문자열타입(varchar, text)
    pub_date = models.DateTimeField(auto_now_add=True) # DateTimeField: 일시타입(datetime, datetime.datetime)
    # auto_now_add:insert 될 때 일시를 자동 입력.
    
    def __str__(self):
        return f"{self.id}.{self.question_text}"

    # db 테이블 생성 쿼리로 위 코드를 비교
...
class Choice(models.Model):
    ...
    question = models.ForeignKey(
        ...
        on_delete=models.CASCADE # 참조 값이 삭제된 경우 어떻게 할지. -> cascade:삭제
    )# FK -> Question의 id를 참조.

    def __str__(self):
        return f"{self.id}.{self.choice_text}"

# 모델 클래스 정의한 후에 Database에 적용. 터미널 명령어 실행
# Project Root >    python manage.py makemigrations # 모든 app들에 적용
#                   python manage.py makemigrations polls # polls app에 적용
