...
col1, col2 = st.columns(2) # 가로를 동일한 width를 가진 동일한 컨테이너를 만들어줌.
# print(type(col1))
col11, col12 = col1.columns(2) #col1에 두 컨테이너를 만듬. col2는 사용 안하고 비워둠.
col11.title("제목")
col11.header("중제목")
col11.subheader("소제목")
col11.text("일반 글1")
...
@st.cache_data  #캐시. @는 데코레이터 라고 함. 주석같은 개념. 캐시데이터는 함수에 붙여줘야함
def get_data():
    print("get_data")
    df = pd.read_csv("data/boston_housing.csv") #파일에 있는 내용을 읽어서 리턴
    return df.head(15)


st.divider()
data = get_data() #처음에 읽어온 것을 계속 사용.
st.title("보스톤 지역 주거지역 정보")
btn = st.button("정보 조회")
if btn:
    st.dataframe(data)
else:
    st.write("버튼클릭하면 보스톤 지역 정보 볼 수 있음")
